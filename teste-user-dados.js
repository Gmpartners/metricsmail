...
// Execução: node teste-user-dados.js
...
async function createCampaigns(accounts) {
    ...
    for (const data of campaignData) {
        ...
        campaigns.push({
            ...
            externalId: `${data.provider}_campaign_${new ObjectId().toString().substring(0, 6)}`,
            status: data.status,
    ...
    for (const campaign of campaigns) {
        ...
        for (let i = 1; i <= templatesCount; i++) {
            ...
            emails.push({
                ...
                externalId: `${account.provider}_email_${new ObjectId().toString().substring(0, 6)}`,
    ...
    for (const email of emails) {
        ...
        for (let i = 0; i < contacts.length; i++) {
            ...
            events.push({
                ...
                externalId: `event_send_${email.provider}_${i}`
            });
            totalEvents++;
            ...
            if (isBounce) {
                ...
                events.push({
                    ...
                    contactId: contact.id,
                    provider: email.provider,
                    externalId: `event_bounce_${email.provider}_${i}`,
            ...
            events.push({
                ...
                contactId: contact.id,
                provider: email.provider,
                externalId: `event_delivery_${email.provider}_${i}`
            });
            totalEvents++;
            ...
            if (isOpened) {
                ...
                events.push({
                    ...
                    contactId: contact.id,
                    provider: email.provider,
                    externalId: `event_open_${email.provider}_${i}`,
                ...
                if (isClicked) {
                    ...
                    events.push({
                        ...
                        contactId: contact.id,
                        provider: email.provider,
                        externalId: `event_click_${email.provider}_${i}`,
                        ipAddress: `192.168.${randomInt(1, 255)}.${randomInt(1, 255)}`,
                        userAgent: `Mozilla/5.0 (${os}) ${browser}/${randomInt(70, 120)}.0`,
                        url: clickedUrl,
                ...
                if (isUnsubscribed) {
                    ...
                    events.push({
                        ...
                        contactId: contact.id,
                        provider: email.provider,
                        externalId: `event_unsub_${email.provider}_${i}`
                    });
                    totalEvents++;
                }
                ...
                if (isComplaint) {
                    ...
                    events.push({
                        ...
                        contactId: contact.id,
                        provider: email.provider,
                        externalId: `event_complaint_${email.provider}_${i}`
                    });
                    totalEvents++;
                }
            }
...
async function createMetrics(campaigns, emails) {
    ...
    const createdMetrics = await Metrics.insertMany(metrics);
